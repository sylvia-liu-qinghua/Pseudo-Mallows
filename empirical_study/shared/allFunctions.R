...
generate_all_Vs<-function(n){
  if(n%%2 == 1){
    tmp<-data.frame(t(matrix(data = c(n:2),ncol = 2, byrow = TRUE)))
    
  }else if(n%%2 == 0){
    tmp<-data.frame(t(matrix(data = c(n:1),ncol = 2, byrow = TRUE)))
  }
  left = expand.grid(tmp) 
  right<-vector()
  
  if(n%%2==0){
    for(i in 1:dim(left)[2]){
      right<-cbind(right,4*(n/2-i+1)-1 - left[,i] )
    }
    V_set = cbind(left,right[,(n/2):1])
    
  }else if(n%%2 == 1){
    #left<-cbind(left,1)
    for(i in 1:dim(left)[2]){
      right<-cbind(right,4*((n+1)/2-i)+1 - left[,i] )
    }
    V_set = cbind(left,1, right[,((n-1)/2):1 ])
  }
  return(V_set)
}
