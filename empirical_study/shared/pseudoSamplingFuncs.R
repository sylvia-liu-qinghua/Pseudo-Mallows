...
createClickData<-function(data,lambda,method){
  ...
  if(method == "poisson"){
    for(i in 1:N){
      clicking[i,]<-as.numeric(clicking[i,]<=min(max(3,rpois(1,lambda)),n))
    }
  }
  return(clicking)
}
...
sampleRhoFull<-function(rho,alpha0){
  n<-length(rho)
  support<-1:n
  user<-rep(0,n)
  In<-generateVOrderings(rho)
  for(i in 1:n){
    i_curr<-which(In==i)
    dist<-sapply(support, oneDimfootrule, Rs=rho[i_curr])
    log_num<-(-alpha0/(n)*(dist)) - max(-alpha0/(n)*(dist)) 
    log_denom<- log(sum(exp(log_num)))
    probs<-exp((log_num-log_denom))
    rand<-runif(1)
    indOfCdf<-length(support)+1-sum(rand<=cdfForSamples(probs))
    user[i_curr]<-support[indOfCdf]
    support<-setdiff(support,user[i_curr])
  }
